<?php
 ...
 $sql="SELECT * FROM `tbl_vendor`,`tbl_login` WHERE tbl_vendor.Login_id=tbl_login.Login_id and tbl_vendor.Login_id='$id';";			
...
while($row = $result->fetch_assoc()) {
  ...
  if(isset($_POST['submit']))
  {
	...
	$sql1=mysqli_query($connection,"UPDATE `tbl_vendor` SET `V_name`='$name',`V_mobile`='$mobileno',`V_email`='$email',`V_address`='$address',`V_houseno`='$houseno',`V_street`='$street',`V_pincode`='$pincode' WHERE `Login_id`='$id'");
	$sql2=mysqli_query($connection,"UPDATE `tbl_login` SET `Username`='$email',`Password`='$password' WHERE `Login_id`='$id'");
	header("Refresh:0; url=Vprofile.php");
  }
?>
...
function pclear(){
 ...
 <form method='post' action="Vprofile.php">
                                            <div class="form-row">
